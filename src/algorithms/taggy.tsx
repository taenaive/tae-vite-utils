...
//   if (typeof text === "string") {
    ...
    let elements: ({ token: string, type: string, } | string | boolean)[] = [];
    ...
    for (let e = 0; e < elements.length; e++) {
      ...
      if (elements[e].hasOwnProperty('token')) {
        ...
        for (let i = e + 1; i < elements.length; i++) {
          ...
          if (
            ...
            (elements[i] as { token: string, type: string, }).type === (elements[e] as { token: string, type: string, }).type
          ) {
            (elements[e] as { token: string, type: string, }).token += " " + (elements[i] as { token: string, type: string, }).token;
            elements[i] = false;
          }
    ...
    const elements2: ({ token: string, type: string, } | string )[] = elements.filter((val) => !!val) as ({ token: string, type: string, } | string )[];
